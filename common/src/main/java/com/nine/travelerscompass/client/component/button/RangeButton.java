...
public class RangeButton<T extends Number> extends BaseButton implements RangeButtonHandler<T> {
    ...
    public void set(T value){
        CompassProperties.putFromClient(supplier.get(), data, value);
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        boolean changed = RangeUtils.changeValue(this, delta < 0, ctrlPressed);
        if (changed){
            updateState();
        }
    ...
    public void refreshTooltip(){
        ...
        components.add(Component.translatable("tooltip.travelerscompass.settings."
                + data.getKey()).withStyle(ChatFormatting.WHITE));
        ...
        if (shiftPressed){
            Component desc = Component.empty().append(ClientUtils.DESC_ARROW)
                    .append(Component.translatable("tooltip.travelerscompass.settings." + data.getKey() + ".desc").withStyle(ChatFormatting.GRAY));
            components.add(desc);
        }
        MutableComponent value = Component.literal(String.valueOf(cached));
        ClientUtils.setColor(value, ClientUtils.SOFT_GRAY);
        components.add(Component.translatable("tooltip.travelerscompass.settings." + data.getKey() +".value", value).withStyle(ChatFormatting.GRAY));
