...
public record LootrContainerLocationObject(BlockPos blockPos, int slotIndex, boolean priority, int containerSlotIndex,
	...
	public static Identifier TYPE = Identifier.fromNamespaceAndPath(TCCommon.MODID, "lootr_container_location_codec");
	...
	public ILocationObject withPriority(boolean value) {
		...
		return new LootrContainerLocationObject(blockPos, slotIndex, value, containerSlotIndex, descriptionId, contentId, blockId);
	}

	@Override
	public ILocationObject withBlockPos(BlockPos pos) {
		if (pos.equals(blockPos)) return this;
		return new LootrContainerLocationObject(pos, slotIndex, priority, containerSlotIndex, descriptionId, contentId, blockId);
	}
